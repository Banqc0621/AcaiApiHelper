...
public class ApiDefinition {
    ...
    private String consumes = RestAutoLabConstants.DEFAULT_CONTENT_TYPE;
    /** 响应Content-Type（如 application/json） */
    private String produces = RestAutoLabConstants.DEFAULT_CONTENT_TYPE;
    /** 接口源码文件绝对路径（用于代码跳转） */
    ...
    private String source = RestAutoLabConstants.API_SOURCE_AUTO;
    /** 扫描/创建时间戳（毫秒） */
    ...
    /** 变更标记（用于API变更检测） */
    private String changeMarker = RestAutoLabConstants.CHANGE_NONE;
    /** 接口标签列表（用于自定义分组） */
    ...
    public void setChangeMarker(String marker) { this.changeMarker = marker != null ? marker : RestAutoLabConstants.CHANGE_NONE; }
    ...
    public boolean isAutoDetected() {
        return RestAutoLabConstants.API_SOURCE_AUTO.equals(source);
    }

    /** 判断状态码是否符合预期（用于测试断言） */
    public boolean isStatusCodeExpected(int code) {
        if (expectedStatusCodes == null || expectedStatusCodes.isEmpty()) {
            return code >= RestAutoLabConstants.HTTP_SUCCESS_MIN
