...
/**
 * 收藏文件夹管理面板 —— 内嵌于工具窗口，非弹出框。
 *
 * <p>两级树：文件夹 → 接口。所有操作通过右键菜单触发：</p>
 * <ul>
 *   <li>文件夹节点：新建/重命名/删除文件夹、添加接口、AI生成参数、批量测试</li>
 *   <li>接口节点：编辑参数、移动到…、复制到…、移除、取消警示</li>
 * </ul>
 *
 * <p>支持拖拽接口到文件夹（移动）。同一接口可存在于多个文件夹（用「复制到」），
 * 同一文件夹内接口唯一。失败接口标红，可手动取消警示。</p>
 */
public class StarredFolderPanel extends JPanel {
    ...
    /** apiKey(uniqueKey) -> ApiDefinition 解析表 */
    ...
    public StarredFolderPanel(@NotNull Project project) {
    ...
    private void rebuildTree() {
        ...
        for (StarredFolder folder : folders) {
            ...
            for (String apiKey : folder.getApiKeys()) {
                ...
                if (api != null) {
                    folderNode.add(new DefaultMutableTreeNode(new ApiUserObject(api, folder.getId())));
                }
            }
            root.add(folderNode);
        }
    ...
    private void doCopyTo() {
        ...
        List<StarredFolder> folders = folderService.loadFolders().stream()
                .filter(f -> !f.getId().equals(auo.folderId)).collect(Collectors.toList());
    ...
    private void doEditParams() {
        ...
        if (editable.isEmpty()) {
            for (ApiParameter p : api.getParameters()) editable.put(p.getName(), "");
        }
    ...
    private final class PopupAdapter extends MouseAdapter {
        ...
        private void maybeShow(MouseEvent e) {
            ...
            if (row < 0) {
                ...
                g.add(action("新建文件夹", AllIcons.Actions.NewFolder, StarredFolderPanel.this::doNewFolder));
                ActionPopupMenu pm = ActionManager.getInstance().createActionPopupMenu(ActionPlaces.POPUP, g);
                pm.getComponent().show(tree, e.getX(), e.getY());
                return;
            }
            ...
            if (uo instanceof FolderUserObject) {
                ...
                group.addSeparator();
                group.add(action("AI生成参数", AllIcons.Actions.Lightning, StarredFolderPanel.this::doBatchAiGen));
                group.add(action("批量测试", AllIcons.Actions.Execute, StarredFolderPanel.this::doBatchTest));
            } else if (uo instanceof ApiUserObject) {
                group.add(action("编辑参数", AllIcons.Actions.EditSource, StarredFolderPanel.this::doEditParams));
                group.add(action("移动到…", AllIcons.Actions.MoveTo2, StarredFolderPanel.this::doMoveTo));
                group.add(action("复制到…", AllIcons.Actions.Copy, StarredFolderPanel.this::doCopyTo));
                group.add(action("移除", AllIcons.Actions.GC, StarredFolderPanel.this::doRemoveApi));
                group.addSeparator();
                group.add(action("取消警示", AllIcons.Actions.QuickfixBulb, StarredFolderPanel.this::doClearWarning));
            }
    ...
    private static AnAction action(String text, Icon icon, Runnable run) {
    ...
    private final class ApiDragTransferHandler extends TransferHandler {
        ...
        @Override public boolean importData(TransferHandler.TransferSupport support) {
            ...
            try {
                ...
                SwingUtilities.invokeLater(StarredFolderPanel.this::rebuildTree);
                return true;
    ...
    private static final class ParamsEditor extends com.intellij.openapi.ui.DialogWrapper {
    ...
    private static final class AddApiPicker extends com.intellij.openapi.ui.DialogWrapper {
        private final java.util.List<ApiDefinition> candidates;
